...
The notebook `Computing_eta_inv_AF_spaces.ipynb` carries out the computations of Section 5.2 (*Spectra of deformed Dirac operators*) of the paper, for the Aloff–Wallach space $N_{k,l} = SU(3)/S^1_{k,l}$. Their output is what Lemma 5.2 rests on: the spectral flow terms are $J_D = 0$ and $J_B = -2$.
...
running from the reductive operator $\widetilde{B} = B^{1/3,0}$ at $\mu = 0$ to the Levi-Civita odd signature operator $B^{1/2,1/2}$ at $\mu = \tfrac{1}{2}$. So $B_0$ is the *difference* between the two operators. There are two stages:

1. The spectrum of $B_0$ on $S \otimes S$ bounds how far the deformation can move an eigenvalue. Comparing $\lambda_{\max}\big(\tfrac12 B_0\big)$ against the lower bound for $({}^\gamma\widetilde{B})^2$ shows that no eigenvalue can cross zero along the path except in the **trivial** representation $\gamma_0$.
...
Everything is done in exact symbolic arithmetic in $k$ and $l$ — there is no numerics and no floating point.
...
```
jupyter lab Computing_eta_inv_AF_spaces.ipynb
```

Then *Kernel → Restart Kernel and Run All Cells*. The cells are meant to be run in order, top to bottom. Runtime is around 30 minutes.
...
3. **The deformation term $B_0$.** We assemble $B_0 = \sum_i c_i\big(\frac{1}{3}\widetilde{\mathrm{ad}}_i + \hat{\widetilde{\mathrm{ad}}}_i\big)$ as a $64\times 64$ matrix on $S\otimes S$, as `B_left + B_right`.
...
6. **The trivial representation: ${}^{\gamma_0}B^{1/2,1/2}$.** We form `S = B_left + B0/2` and restrict it to the same $H$-invariant subspace — which reappears here as the first block of the decomposition, so the run independently confirms that `H_inv` is genuinely invariant. Some blocks have eigenvalues SymPy cannot express in closed form, but only their *signs* are needed: each block is self-adjoint, so its eigenvalues are real and the sign counts follow from Descartes' rule applied to the characteristic polynomials. The $10\times10$ block is handled by factoring its characteristic polynomial as
...
- $k, l$ are declared **positive integers**. The paper's results cover general pairs $(k,l)$; the remaining cases follow from the sign and symmetry arguments there rather than from this notebook.
...
- The notebook treats the *generic* case. The exceptional spaces $N_{1,1}$ and $N_{1,0}$ have a larger $H$-invariant subspace of $S\otimes S$ and are handled separately in the paper's appendix. However, they can be computed from the notebook's results — one just needs the basis for the $H$-invariant subspace in each exceptional case.

## Normalisation

- The $\widetilde{\mathrm{ad}}_i$ are **not** divided by 4, because doing so makes SymPy fall back to floating point. Every eigenvalue printed for $B_0$ is therefore 4× the paper's value: the maximum $8\sqrt{2}$ corresponds to $2\sqrt{2}$ there.
